...
dev.off()
setwd("/Users/alex/Documents/Documents - Alex’s MacBook Pro/Documents/My Projects/BADDIE/Sequencing/16S")
```
...
# Loading in files
```{r}
zotutab1 <- read_delim("feature_table_16S_BADDIE.txt", delim = "\t")
meta1 <- read_delim("Metadata2.txt", delim = "\t")
tax1 <- read_delim("taxonomy_table_16S_BADDIE.txt", delim = "\t", col_names = FALSE)
tax1 <- tax1[-1,-3]
taxcut1 <- tax1 %>%
  dplyr::rename("#OTU ID" = "X1", "taxonomy" = "X2")
```

# Removing rare taxa (less than 20 reads)
```{r}
otutotals1 <- mutate(.data = zotutab1, otutotal = rowSums(zotutab1[,2:37]))
otunotrare1 <- otutotals1 %>%
  filter(otutotal > 20)
#Filters out 1659 taxa
```
...
# Rarefy data
```{r}
input_rar <- single_rarefy(input, 11000)
...
```{r}
#Summarize taxonomy
bac_taxa = summarize_taxonomy(input_rar, level=2)
metaplot = as.data.frame(input_rar$map_loaded)
#Create barplot
bac_comp = plot_taxa_bars(
  tax_table = bac_taxa,
...
#By panel location
plot_diversity(input_rar, "Panels", metric = "richness")
plot_diversity(input_rar, "Panels", metric = "shannon")
```
...
library(broom)
shan_Microsite_tidy <- tidy(shan_Microsite_anova)
shan_Microsite_tidy

#Comparing shannon by microsite
...
# Panel location (within v outside)
shan_panel_glm <- glm(shan ~ Panels, data = shan_df)
shan_panel_anova <- car::Anova(shan_panel_glm, test.statistic = "F")
...
ShanPlot =
  ...
  scale_fill_manual(values = c("lightcyan2",  "grey69", "indianred4", "aquamarine3", "cornflowerblue")) +
  labs(
    title = "16S OTU Shannon Diversity",
...
plot_ordination(input, ord1, 'Panels', hulls = TRUE)
```
...
x <- cbind(o16S$map_loaded, t(o16S$data_loaded))
head(x)
...
# Permanova by Panels
permanova_panels <- adonis2(x[,6:ncol(x)] ~ Panels,
                       data = x)
permanova_panels
```
...
pair.mod<-pairwise.adonis(t(otuTAB),factors=mta$Microsite, p.adjust.m="bonferroni")
pair.mod

# By within v outside panels
mta$Panels <- as.factor(mta$Panels)
pair.mod<-pairwise.adonis(t(otuTAB),factors=mta$Panels)
pair.mod
```
...
ordiellipse(Ordination.model1, groups = mta$Microsite, draw = "polygon",col=c("grey69","cornflowerblue","aquamarine3","indianred4","orange"))
...
#CAPS by within v outside panels
```{r}
otuTAB <- as.data.frame (otuTAB)
otuTABtranspose<-t(otuTAB)
mta<-input$map_loaded
...
o.dist <-vegdist (otuTABtranspose)
Ordination.model1 <- CAPdiscrim (o.dist ~ Panels, data = mta,
                                 dist="bray", axes=2, m=0, add=FALSE)

plot.new()
plot1<-ordiplot(Ordination.model1,type="none")
plot1
ordisymbol(plot1, mta, "Panels", legend=TRUE,legend.x="topleft")
ordiellipse(Ordination.model1, groups = mta$Panels, draw = "polygon",col=c("grey69","cornflowerblue","aquamarine3"))
```
...
plot_ts_heatmap(tax_sum_2, input$map_loaded, 0.01, 'Microsite')

#By inside, outside, between panels by phylum
plot_ts_heatmap(tax_sum_5, input$map_loaded, 0.01, 'Panels')
```

# Volcano plots
# calculating logfold changes
```{r}
# load packages
library(DESeq2)
# get tables
...
# results
volres <- results(voldat2, contrast = c('Panels', 'Within', 'Outside')) # first factor = up, second factor = down
...
# Phylum level volcano plot
```{r}
vol_plot <- volrestax %>%
  ggplot(aes(x = log2FoldChange, y = -log10(padj), color = taxonomy2)) +
  geom_point(size = 1.2) +
  labs(x = 'log2(Fold Change)', y = '-log10(adjust p-value)', color='Bacterial Phylum') +  
  ...
  xlim(-8, 8) + ylim(0, 33) + ggtitle ("Taxa Depleted vs Enriched in Array")
vol_plot

#Within array shows depletion of planctomycetes, some Chloroflexi, and some Actinobacteria
#Add labels of which taxa are enriched/depleted
```
...
taxa_by_fam = taxa_summary_by_sample_type(tax_sum_families, input$map_loaded, type_header = 'SampleID', filter_level = 0.05, test_type = 'KW')
taxa_by_fam$RA <- rowMeans(taxa_by_fam[ , c(4,39)], na.rm=TRUE)
```
...
sums = merge(features, tax, by="row.names") %>%
  mutate(otutotal = rowSums(sums[,2:37])) %>%
  arrange(desc(otutotal))
#Separate top 10% of taxa by RA 
dominant_taxa = sums[1:144,]

#There are 571834 total reads of 16S samples
total_reads = as.data.frame(sums[,45])
colnames(total_reads)[1] <- "sums"
sum(total_reads$sums)

#Top 10% of taxa account for 303719 total reads
dominant_reads = as.data.frame(dominant_taxa[,45])
...
#Reading in tables
otu_mat<- read_excel("/Users/alex/Documents/Documents - Alex’s MacBook Pro/Documents/My Projects/BADDIE/Sequencing/16S/feature_table_16S_BADDIE.xlsx")
colnames(otu_mat)[1] <- "otu"
tax_mat<- read_delim("/Users/alex/Documents/Documents - Alex’s MacBook Pro/Documents/My Projects/BADDIE/Sequencing/16S/taxonomy_table_16S_BADDIE_MA.txt")
...
#Looking at shared taxa across microsites
```{r}
...
sample_data(baddie_filt)$Microsite = factor(sample_data(baddie_filt)$Microsite, levels = c("Control", "Between", "Beneath", "East", "West"))
#ANCOM
...
tab_p = res$p_val
col_name = c("Taxon", "Intercept", "Control-Between", "Control-Beneath", "Control-East", "Control-West")
...
ANCOM_all_micro_sig= 
  ...
  as.data.frame %>% 
  filter(MicrositeBetween<p_thresh) %$%
  taxon
...
ANCOM_all_micro_results=
  ...
  rename("LFC"="MicrositeBetween") %>%
  mutate(Enrichment=ifelse(LFC<0, "Control Enriched", "Between Enriched"))
...
ANCOM_all_micro_results %>%
  ...
  coord_flip() +
  theme_minimal() +
  ggtitle("Control vs. Between Panels - Top 40 enriched taxa (16S)") +
...
ANCOM_all_micro_results %>%
  ...
  theme_minimal() +
  ggtitle("East vs. West Edge - Enriched Prokaryotic Taxa") +
  geom_hline(yintercept=0) +
...
ANCOM_all_micro_results %>%
  ...
  theme_minimal() +
  ggtitle("Beneath vs. Control - Enriched Prokaryotic Taxa") +
  geom_hline(yintercept=0) +
  theme(plot.title = element_text(hjust = 0.5, size = 25)) +
  theme(legend.key.size = unit(1.5, 'cm'), #change legend key size
        legend.title = element_text(size=12), #change legend title font size
...
MasterBac = read_excel("/Users/alex/Documents/Documents - Alex’s MacBook Pro/Documents/My Projects/BADDIE/Data/Soil_Master_Bacteria.xlsx")

#Changing columns to numeric
MasterBac[,3:24] <- sapply(MasterBac[,3:24],as.numeric)
#Removing ITS columns
MasterBac = MasterBac[,-c(16:17)]

#Run correlations
print(cor(MasterBac[, c(3:22]))

#Calculate Spearman correlation coefficients
cor_matrix <- cor(MasterBac[, c(3:22)], method = "spearman")
...
heatmaply_cor(x = cor(MasterBac[, c(3:22)]), xlab = "", 
